...
local SafeDistance = getgenv().SafeDistance or 15 -- Khoảng cách an toàn tránh conflict

-- Xác định acc hiện tại
local CurrentPet = nil
local farmMode = "Checker"
local farmerAccounts = {} -- Danh sách tất cả acc farmer
...
local function IsPlotOwnedByFarmer(plot)
	-- Kiểm tra xem plot có thuộc về farmer account không
	...
	for _, farmerName in ipairs(farmerAccounts) do
		...
		if farmerPlayer then
			local farmerPlot = GetMyPlot() -- Lấy plot của farmer
			-- So sánh plot (có thể dùng tên hoặc properties khác)
			if plot == farmerPlot then
				return true
			end
		end
	end
...
local function IsAnyFarmerNearPet(petPosition)
	-- Kiểm tra xem có farmer nào đang ở gần pet không
	if not petPosition then return false end
	
	for _, farmerName in ipairs(farmerAccounts) do
		if farmerName == player.Name then continue end -- Bỏ qua chính mình
...
local function ScanAllPlots()
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			-- ❌ BỎ QUA plot của các farmer khác
			...
			for _, pet in ipairs(plot:GetChildren()) do
				if pet.Name == CurrentPet then
					...
					if petPos then
						-- ⚠️ KIỂM TRA có farmer nào đang ở gần không
