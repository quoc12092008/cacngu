...
discordText.Text = "discord.gg/chuoiHub"
...
title.Text = "Pet Farm Stats Checker"
...
rightTitle.Text = "Pets in My Plot"
...
-- 🟢 HÀM MỚI: Kiểm tra xem plot có thuộc về acc trong AccHold không
local function IsPlotOwnedByFarmer(plot)
	for _, acc in ipairs(AccHold) do
		-- Kiểm tra tên plot hoặc owner (tùy game structure)
		if plot.Name:find(acc.AccountName) then
			return true
		end
	end
	return false
...
-- 🟢 HÀM MỚI: Claim pet bằng Attribute
local function TryClaimPet(pet)
	-- Kiểm tra xem pet đã bị claim chưa
	local claimedBy = pet:GetAttribute("ClaimedBy")
	if claimedBy then
		print("[SKIP] " .. pet.Name .. " đang được " .. claimedBy .. " lấy")
		return false
	end
	
	-- Claim pet
	pet:SetAttribute("ClaimedBy", player.Name)
	pet:SetAttribute("ClaimTime", os.time())
	print("[CLAIMED] " .. player.Name .. " đã claim " .. pet.Name)
	return true
end

-- 🟢 HÀM MỚI: Release pet sau khi lấy xong
local function ReleasePet(pet)
	if pet and pet.Parent then
		pet:SetAttribute("ClaimedBy", nil)
		pet:SetAttribute("ClaimTime", nil)
		print("[RELEASED] " .. player.Name .. " đã release " .. pet.Name)
	end
end

local function HandlePet(pet, myPlot)
	-- Thử claim pet trước
	if not TryClaimPet(pet) then
		return
	end
	...
	-- Release pet sau khi xong (thành công hay thất bại)
	ReleasePet(pet)
	
	if success then
		statusText.Text = "Status : Farming " .. CurrentPet
	else
		statusText.Text = "Status : Failed, retrying..."
	end
end
...
local function ScanAllPlots()
	...
	-- 🟢 FIX: Kiểm tra nhà mình đã có pet chưa
	local myPets = GetPetsInPlot(myPlot)
	for _, petName in ipairs(myPets) do
		if petName == CurrentPet then
			statusText.Text = "Status : Pet already in my plot"
			return
		end
	end
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			-- 🟢 FIX: Bỏ qua plot của các acc farmer khác
			...
			for _, pet in ipairs(plot:GetChildren()) do
				if pet.Name == CurrentPet then
					-- Kiểm tra pet có đang bị claim không
					local claimedBy = pet:GetAttribute("ClaimedBy")
					if not claimedBy then
						HandlePet(pet, myPlot)
						-- Sau khi xử lý 1 pet thì dừng để check lại plot
						return
					end
				end
			end
...
-- 🟢 HÀM MỚI: Cleanup các claim quá cũ (tránh deadlock)
		...
		if plot:IsA("Model") then
			for _, pet in ipairs(plot:GetChildren()) do
				if pet:IsA("Model") then
					local claimTime = pet:GetAttribute("ClaimTime")
					if claimTime and (os.time() - claimTime > 30) then
						-- Claim quá 30s → xóa (có thể acc bị disconnect)
						ReleasePet(pet)
					end
				end
			end
...
if CurrentPet then
	...
	-- Cleanup loop (xóa claim cũ mỗi 15s)
	task.spawn(function()
		while true do
			task.wait(15)
			pcall(CleanupOldClaims)
		end
	end)
else
