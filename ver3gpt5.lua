...
local function HandlePet(pet, myPlot)
	...
	-- 🧠 Nếu pet có bản clone ở workspace thì coi như người khác đã cầm → bỏ qua
	for _, obj in ipairs(workspace:GetChildren()) do
		if obj.Name == pet.Name and obj ~= pet then
	...
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		...
		-- 🔍 Sau khi hold, kiểm tra lại: có clone mới xuất hiện trong workspace không?
		local nowHasClone = false
		for _, obj in ipairs(workspace:GetChildren()) do
			if obj.Name == pet.Name and obj ~= pet then
				nowHasClone = true
				break
		...
		if nowHasClone then
			-- ❌ Pet xuất hiện bản ngoài workspace sau khi hold → bị người khác lấy
			...
			-- ✅ Không có clone ngoài workspace → mình là người cầm thành công
