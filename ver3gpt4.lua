...
local function HandlePet(pet, myPlot)
	...
	if IsGhostPet(pet) then
		print("[👻] Bỏ qua pet bóng mờ:", pet.Name)
		return
	end
	...
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		...
		-- 🧠 Xác minh: nếu có pet trùng tên xuất hiện ở workspace → thành công
		...
		if foundInWorkspace then
			print("[✅] Đã cầm pet thật:", pet.Name)
			...
			print("[❌] Pet bị giành hoặc bóng mờ:", pet.Name)
			statusText.Text = "Status : Pet bị giành → bỏ qua"
			StolenPets[pet] = true
			task.wait(1)
			return
		end
