...
-- 🟡 FIX: Tạo bảng lưu trạng thái pet đang bị claim (đang được acc khác lấy)
if not getgenv().PetClaimStatus then
	getgenv().PetClaimStatus = {}
end
...
local function HandlePet(pet, myPlot)
	...
	-- 🟡 FIX: Nếu pet đang bị acc khác claim thì bỏ qua
	if getgenv().PetClaimStatus[pet] then
		print("[SKIP] " .. pet.Name .. " đang được acc khác lấy.")
		return
	end

	getgenv().PetClaimStatus[pet] = player.Name
	...
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		...
		-- 🟡 FIX: Bỏ đánh dấu khi xong
	getgenv().PetClaimStatus[pet] = nil
...
local function ScanAllPlots()
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			-- 🟡 FIX: Bỏ qua plot của các acc khác trong AccHold (tránh farm pet trong nhà họ)
			for _, acc in ipairs(AccHold) do
				if plot.Name == acc.AccountName then
					continue
				end
			end
...
local function GetPetsInPlot(plot)
	...
	-- Danh sách loại trừ (không phải pet) - check cả chính xác và contain
	local skipList = {
	...
	for _, obj in ipairs(plot:GetChildren()) do
		-- Skip nếu không phải Model
		if not obj:IsA("Model") then
			continue
		end
		
		-- Kiểm tra tên có trong danh sách loại trừ không
		...
		-- Nếu không skip và có HumanoidRootPart/PrimaryPart → là pet
