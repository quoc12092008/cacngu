...
local function CheckMyPlotEmpty()
	...
	for _, pet in ipairs(myPlot:GetChildren()) do
		...
		for _, cfg in ipairs(AccHold) do
			...
			-- Nếu tên pet chứa tên trong config
			if petName:find(cfgName) then
				return false
			end

			-- Nếu petName có chứa bất kỳ mutation nào
			for _, mut in ipairs(mutations) do
				if petName:find(mut) then
					return false
				end
			end
		end
