	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			...
			for _, pet in ipairs(plot:GetChildren()) do
                ...
                if pet.Name == CurrentPet or ((mutation or yin) and player.Name == cfg.AccountName) then
                    match = true
                end
...
---------------------------------------------------------------------
-- 🧩 Check nếu hết pet (hỗ trợ mutation/yin)
---------------------------------------------------------------------
local function CheckMyPlotEmpty()
	...
	for _, pet in ipairs(myPlot:GetChildren()) do
		...
		for _, cfg in ipairs(AccHold) do
			-- Chỉ kiểm tra acc hiện tại
			if cfg.AccountName == player.Name then
				-- ✅ Cho phép tên khớp gần đúng hoặc có mutation/yin
				if pet.Name == cfg.Pet 
					or pet.Name:find(cfg.Pet) 
					or ((mutation or yin) and pet.Name:find(cfg.Pet)) then

					print(("[DEBUG] 🐾 Vẫn còn pet hợp lệ của acc %s: %s | Mutation=%s | YinYang=%s"):format(
						player.Name, pet.Name, tostring(mutation), tostring(yin)
					))
					return false -- còn pet hợp lệ → KHÔNG kick
				end
			end
		end
	end
