...
-- IMPROVED: Handle race condition khi nhiều instance cùng ghi file
local function writeToAccountsFile()
    local player = game.Players.LocalPlayer
    local username = player.Name
    local text = username .. ":Done"
    
    -- Thử appendfile trước (nếu executor hỗ trợ)
    if appendfile then
        local success = pcall(function()
            if isfile("accounts.txt") then
                appendfile("accounts.txt", "\n" .. text)
            else
    ...
    -- Fallback: Retry với random delay để tránh conflict
    local maxRetries = 5
    for attempt = 1, maxRetries do
        local success = pcall(function()
            -- Random delay để giảm conflict
            task.wait(math.random() * 0.3)
            
            if isfile("accounts.txt") then
                local existingContent = readfile("accounts.txt")
                -- Check xem đã có username chưa (tránh duplicate)
                if not string.find(existingContent:lower(), username:lower() .. ":done") then
    ...
    -- Last resort: Ghi vào file riêng
    pcall(function()
        writefile("account_" .. username .. ".txt", text)
        print("✓ Written to account_" .. username .. ".txt")
    end)
end
