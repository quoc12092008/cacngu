...
local myTeamAccounts = {}

for _, cfg in ipairs(AccHold) do
	table.insert(myTeamAccounts, cfg.AccountName)
...
local function IsTeamMate(playerName)
	for _, accName in ipairs(myTeamAccounts) do
		if accName == playerName then
			return true
		end
	end
	return false
...
local function IsAnyTeamMateNearPet(pet, safeDistance)
	safeDistance = safeDistance or 15 -- Khoảng cách an toàn
	
	local petPos = pet.WorldPivot and pet.WorldPivot.Position or pet.Position
	if not petPos then return false, nil end
	
	for _, otherPlayer in ipairs(Players:GetPlayers()) do
		if otherPlayer ~= player and IsTeamMate(otherPlayer.Name) then
			if otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") then
				local otherHRP = otherPlayer.Character.HumanoidRootPart
				local distance = (otherHRP.Position - petPos).Magnitude
...
local function WaitForPetAvailable(pet, maxWaitTime)
	local startTime = tick()
	
	while tick() - startTime < maxWaitTime do
		local isNear, whoIsNear = IsAnyTeamMateNearPet(pet, 15)
		
		if not isNear then
			return true
		end
		
		-- Kiểm tra xem pet còn tồn tại không
		if not pet.Parent then
			return false
		end
		
		statusText.Text = "Status : Waiting for " .. whoIsNear .. "..."
		task.wait(0.5)
	end
...
local function IsPetStillThere(pet)
	return pet and pet.Parent and (pet.WorldPivot or pet.Position)
end
...
local function WalkToPosition(targetPos, checkPetExist)
	...
	for _, wp in ipairs(path:GetWaypoints()) do
		-- Kiểm tra pet còn tồn tại không khi đang đi
		if checkPetExist and not IsPetStillThere(checkPetExist) then
			statusText.Text = "Status : Pet disappeared!"
			return false
		end
...
local function HoldKeyEReal(duration, pet)
	local start = tick()
	while tick() - start < duration do
		-- Kiểm tra pet còn tồn tại không trong lúc hold
		if not IsPetStillThere(pet) then
			statusText.Text = "Status : Pet disappeared while holding!"
			return false
		end
...
local function HandlePet(pet, myPlot)
	-- Bước 1: Kiểm tra có ai đang ở gần không
	local isNear, whoIsNear = IsAnyTeamMateNearPet(pet, 15)
	if isNear then
		statusText.Text = "Status : " .. whoIsNear .. " is farming, waiting..."
		
		-- Đợi tối đa 20 giây
		if not WaitForPetAvailable(pet, 20) then
			statusText.Text = "Status : Pet taken or timeout"
			return
		end
	end
	
	-- Bước 2: Kiểm tra pet còn tồn tại không
	if not IsPetStillThere(pet) then
		statusText.Text = "Status : Pet disappeared"
		return
	end
	...
	-- Bước 3: Đi tới pet (với check pet còn không)
	statusText.Text = "Status : Walking to " .. pet.Name
	if not WalkToPosition(targetPos + Vector3.new(0, 2, 0), pet) then
		statusText.Text = "Status : Failed to reach pet"
		return
	end
	
	-- Bước 4: Kiểm tra lại pet trước khi hold
	if not IsPetStillThere(pet) then
		statusText.Text = "Status : Pet gone before holding"
		return
	end
	...
	local holdSuccess = HoldKeyEReal(HoldTime, pet)
	
	if not holdSuccess then
		statusText.Text = "Status : Hold interrupted"
		return
	end
...
local function ScanAllPlots()
	if isCurrentlyFarming then return end
		...
		if plot:IsA("Model") and plot ~= myPlot then
			for _, pet in ipairs(plot:GetChildren()) do
				if pet.Name == CurrentPet then
					-- Kiểm tra có ai ở gần không
					local isNear, whoIsNear = IsAnyTeamMateNearPet(pet, 15)
					
					if not isNear then
						isCurrentlyFarming = true
						print("[FARM] ✅", player.Name, "farming:", pet.Name)
						
						HandlePet(pet, myPlot)
						
						isCurrentlyFarming = false
						
						-- Random delay để tránh scan đồng thời
						task.wait(math.random(1, 2))
						return
					else
						print("[SKIP] ⏭️", whoIsNear, "is near", pet.Name)
					end
				end
			end
		end
...
local function CountNearbyTeammates()
	local count = 0
	for _, otherPlayer in ipairs(Players:GetPlayers()) do
		if otherPlayer ~= player and IsTeamMate(otherPlayer.Name) then
			if otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") then
...
local function UpdateUI()
	...
	statLabels[7].Text = "Teammates Online: " .. CountNearbyTeammates()
...
if CurrentPet then
	...
	-- Farm loop với random offset
	task.spawn(function()
		-- Random start delay để tránh scan cùng lúc
		local startDelay = math.random(0, 15) / 10
		...
		while true do
			...
			-- Random interval để không scan đồng thời
			local waitTime = CheckInterval + math.random(-5, 5) / 10
			task.wait(waitTime)
		end
	end)
else
...
UpdateUI()
print("✅ Pet Farm UI + Script loaded (Distance-Based Anti-Conflict)!")
