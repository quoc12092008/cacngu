...
local function IsTeammatePlot(plotOwnerName)
	if not plotOwnerName then return false end
	
	for _, cfg in ipairs(AccHold) do
		if cfg.AccountName == plotOwnerName then
			return true
		end
	end
	return false
...
local function WalkToPosition(targetPos)
	...
	if path.Status ~= Enum.PathStatus.Success then
		print("[⚠️] Path computation failed")
		return false
	end
	...
	for _, wp in ipairs(path:GetWaypoints()) do
		...
		if not reached then 
			print("[⚠️] Failed to reach waypoint")
			return false 
		end
	end
	return true
...
local function ClaimPet(petId)
	...
	print("[🔒] Pet claimed:", petId, "by", player.Name)
end

local function ReleasePet(petId)
	ClaimedPets[petId] = nil
	print("[🔓] Pet released:", petId)
end
...
local function HandlePet(pet, myPlot, plotOwner)
	...
	if IsPetBeingClaimed(petId) then
		...
		print("[⏭️] Pet already claimed, skipping...")
		return false
	end
	...
	local targetPos = pet.WorldPivot and pet.WorldPivot.Position or (pet.PrimaryPart and pet.PrimaryPart.Position)
	if not targetPos then 
		print("[❌] Cannot get pet position")
		ReleasePet(petId)
		return false 
	end

	print("[🎯] Targeting pet:", pet.Name, "at position:", targetPos)
	...
	if not WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		print("[❌] Failed to walk to pet")
		ReleasePet(petId)
		return false
	end
	
	print("[✅] Reached pet location")
	...
	print("[⏳] Holding E for", HoldTime, "seconds...")
	...
	-- ✅ SO SÁNH TỐC ĐỘ
	local speedDifference = speedBefore - speedAfter
	print("[📊] Speed difference:", speedDifference)
	local pickedUp = speedDifference > 2
	
	if not pickedUp then
		statusText.Text = "Status : ❌ Failed to pick up (speed unchanged), retrying..."
		print("[⚠️] Không cầm được pet, tốc độ không đổi. Thử lại...")
		ReleasePet(petId)
		return false
	end
	
	-- ✅ ĐÃ CẦM ĐƯỢC PET, VỀ NHÀ
	statusText.Text = "Status : ✅ Pet picked up! Returning home..."
	print("[✅] Đã cầm được pet! Về nhà...")
	
	local homePos = GetHomeSpawn(myPlot)
	if homePos then
		print("[🏠] Walking to home position:", homePos)
		WalkToPosition(homePos + Vector3.new(0, 2, 0))
		
		-- 🎯 SAU KHI VỀ NHÀ: Mua Speed Coil (nếu chưa có)
	...
	ReleasePet(petId)
	print("[✅] Pet handling completed successfully")
	return true
end
...
local function ScanAllPlots()
	...
	local skippedTeammatePlots = 0
	
	print("[🔍] Starting plot scan...")
	
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			scannedPlots = scannedPlots + 1
			
			-- Lấy tên owner từ plot name hoặc thuộc tính
			local plotName = plot.Name
			local plotOwner = nil
			
			-- Thử tìm owner bằng cách check xem player nào có plot này
			for _, otherPlayer in ipairs(Players:GetPlayers()) do
				if otherPlayer ~= player then
					local theirPlot = PlotController.GetPlot(otherPlayer)
					if theirPlot and theirPlot.PlotModel == plot then
						plotOwner = otherPlayer.Name
						break
					end
				end
			end
			
			-- 🚫 KIỂM TRA NẾU LÀ PLOT CỦA TEAMMATE
			if plotOwner and IsTeammatePlot(plotOwner) then
				print("[🚫] Skipping teammate plot:", plotOwner)
				skippedTeammatePlots = skippedTeammatePlots + 1
			else
				-- TÌM PET TRONG PLOT
				for _, obj in ipairs(plot:GetChildren()) do
					if obj:IsA("Model") and obj.Name == CurrentPet then
						local petId = obj:GetFullName()
						if not IsPetBeingClaimed(petId) then
							print("[✅] Found available pet:", obj.Name, "in plot:", plotOwner or "Unknown")
							...
							print("[⏭️] Pet already claimed:", obj.Name)
						end
					end
				end
			end
	...
	print("[📊] Scan complete: Scanned", scannedPlots, "plots, Skipped", skippedTeammatePlots, "teammate plots, Found", #availablePets, "available pets")
...
local function UpdateUI()
	...
	statLabels[7].Text = "Speed Coil: " .. (speedCoilActive and "✅ Active" or "❌ Inactive")
...
if CurrentPet then
	print("==============================================")
	print("[ACC FARM] 🎯", player.Name, "→ Target pet:", CurrentPet)
	print("[ACC FARM] Hold Time:", HoldTime, "seconds")
	print("[ACC FARM] Check Interval:", CheckInterval, "seconds")
	print("==============================================")
	statusText.Text = "Status : Farming " .. CurrentPet
	
	-- Farm loop với random selection và debug
	task.spawn(function()
		task.wait(2) -- Chờ 2s để đảm bảo mọi thứ đã load
		while true do
			...
			if not success then
				warn("[❌] Error in farm loop:", err)
			end
			
			print("[⏰] Waiting", CheckInterval, "seconds before next scan...\n")
...
UpdateUI()
print("✅ Pet Farm UI + Anti-Conflict + Speed Check + Debug loaded!")
print("📝 Check console (F9) for detailed logs")
