...
local function HandlePet(pet, myPlot)
	...
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		...
		-- 🧠 Kiểm tra nếu có pet trùng tên trong workspace (ngoài plot)
		...
		if foundInWorkspace then
			print("[✅] Đã cầm pet thành công:", pet.Name)
			...
			-- ❌ Không thấy pet ngoài workspace → bị giành hoặc fail hold
			print("[❌] Pet bị giành hoặc chưa nhặt được:", pet.Name)
