...
local function HandlePet(pet, myPlot)
	...
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		...
		-- ⚠️ Tắt Speed Coil khi chuẩn bị nhặt pet
		speedCoilActive = false
		
		-- Ghi nhận tốc độ trước khi nhấn giữ E
		local speedBefore = GetCurrentSpeed()
		...
		-- Kiểm tra tốc độ sau khi hold
		local speedAfter = GetCurrentSpeed()
		if math.abs(speedAfter - speedBefore) < 1 then
			print("[❌] Pet bị giành hoặc không cầm được:", pet.Name)
			StolenPets[pet] = true
