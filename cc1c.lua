(function() 
    ...
    function mmb() 
        ...
        DiscordLabel.Position = UDim2.new(0.5, 0, 0.02, 0)
        DiscordLabel.Size = UDim2.new(0, 300, 0, 30)
        DiscordLabel.AnchorPoint = Vector2.new(0.5, 0)
        DiscordLabel.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
        DiscordLabel.BorderColor3 = Color3.fromRGB(255, 200, 0)
        DiscordLabel.BorderSizePixel = 2
        DiscordLabel.Text = "discord.gg/chuoihub"
        ...
        StatusFrame.Position = UDim2.new(0.5, 0, 0.08, 0)
        StatusFrame.Size = UDim2.new(0, 450, 0, 70)
        StatusFrame.AnchorPoint = Vector2.new(0.5, 0)
        StatusFrame.BackgroundColor3 = Color3.fromRGB(20, 25, 35)
        StatusFrame.BorderColor3 = Color3.fromRGB(255, 200, 0)
        ...
        Interface.Instances.StatusLabel = StatusLabel
        ...
        Divider.Position = UDim2.new(0.1, 0, 0.18, 0)
        ...
        StatsTitle.Position = UDim2.new(0.1, 0, 0.25, 0)
        StatsTitle.Size = UDim2.new(0.35, 0, 0, 30)
        ...
        for _, config in ipairs(statsConfig) do
            local StatLabel = Instance.new("TextLabel")
            StatLabel.Parent = MainFrame
            StatLabel.Position = UDim2.new(0.1, 0, config.yPos, 0)
            StatLabel.Size = UDim2.new(0.35, 0, 0, 25)
            StatLabel.BackgroundTransparency = 1
            StatLabel.Text = config.key .. ": 0"
            StatLabel.TextColor3 = Color3.fromRGB(220, 220, 220)
            StatLabel.TextSize = 18
            StatLabel.Font = Enum.Font.Gotham
            StatLabel.TextXAlignment = Enum.TextXAlignment.Left
            
            statsLabels[config.key] = StatLabel
            Interface.Instances[config.key] = StatLabel
        end

        -- Account Items Title
        local ItemsTitle = Instance.new("TextLabel")
        ItemsTitle.Parent = MainFrame
        ItemsTitle.Position = UDim2.new(0.55, 0, 0.25, 0)
        ItemsTitle.Size = UDim2.new(0.35, 0, 0, 30)
        ...
        -- Items Configuration
        local itemsConfig = {
            {name = "GodHuman", column = 1, row = 1},
            {name = "Soul Guitar", column = 1, row = 2},
            {name = "Cursed Dual Katana", column = 2, row = 1},
            {name = "Mirror Fractal", column = 2, row = 2},
            {name = "Valkyrie Helm", column = 3, row = 1},
            {name = "Pull Lever", column = 3, row = 2}
        }

        for _, item in ipairs(itemsConfig) do
            local xPos = 0.1 + ((item.column - 1) * 0.3)
            local yPos = 0.8 + ((item.row - 1) * 0.08)
            
            -- Red Dot
            local Dot = Instance.new("Frame")
            Dot.Name = item.name .. "_Dot"
            Dot.Parent = MainFrame
            Dot.Position = UDim2.new(xPos, 0, yPos, 8)
            ...
            Interface.Items[item.name] = {Dot = Dot, HasItem = false}
            
            -- Item Label
            local ItemLabel = Instance.new("TextLabel")
            ItemLabel.Parent = MainFrame
            ItemLabel.Position = UDim2.new(xPos + 0.03, 0, yPos, 0)
            ItemLabel.Size = UDim2.new(0.25, 0, 0, 25)
        ...
        -- Hàm cập nhật Items
        function Interface:UpdateItem(itemName, hasItem)
            if self.Items[itemName] then
                self.Items[itemName].HasItem = hasItem
                -- Green nếu có, Red nếu không
                self.Items[itemName].Dot.BackgroundColor3 = hasItem and 
                    Color3.fromRGB(60, 255, 60) or Color3.fromRGB(255, 60, 60)
            end
        end

        -- Hàm cập nhật Race
        function Interface:UpdateRace(race)
            statsLabels.Race.Text = "Race: " .. race
        end
        ...
        spawn(function()
            ...
            game:GetService("Players").LocalPlayer.PlayerScripts:WaitForChild('IslandLOD', 9999):Destroy()
                ...
                -- GỌI HÀM mmb() ĐỂ TẠO INTERFACE
                Interface = mmb()
                alert("Interface created", "UI loaded successfully")


                -- ĐỊNH NGHĨA SetText NGAY SAU KHI TẠO INTERFACE
                function SetText(Name, Text)
                    if not Interface or not Interface.Instances then 
                        print("[SetText] Interface chưa sẵn sàng")
                        return 
                    end
                    
                    if Name == "MainTextLabel" or Name == "Task1" then
                        if Interface.Instances.StatusLabel then
                            Interface.Instances.StatusLabel.Text = "Status : " .. Text
                        end
                    elseif Name == "Task2" then
                        -- SubTask - có thể hiển thị hoặc bỏ qua
                    elseif Name == "Currencies" then
                        local level = string.match(Text, "Level</font>: (%d+)")
                        local beli = string.match(Text, "Beli</font>: ([%d,]+)")
                        local frag = string.match(Text, "Fragments</font>: (%d+)")
                        
                        if level and Interface.Instances.Level then
                            Interface.Instances.Level.Text = "Level: " .. level
                        end
                        if beli and Interface.Instances.Beli then
                            Interface.Instances.Beli.Text = "Beli: " .. beli
                        end
                        if frag and Interface.Instances.Frag then
                            Interface.Instances.Frag.Text = "Frag: " .. frag
                        end
                    end
                end
