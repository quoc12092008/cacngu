...
-- ⚠️ Danh sách pet đang bị giành (để tránh loop lại)
local StolenPets = {}

-- 🧠 Kiểm tra pet có thuộc plot của acc khác trong AccHold không
local function IsPetInFriendPlot(pet)
	local plot = pet:FindFirstAncestorWhichIsA("Model")
	if not plot then return false end
	for _, cfg in ipairs(AccHold) do
		if plot.Name == cfg.AccountName then
			return cfg.AccountName ~= player.Name
		end
	end
	return false
end

-- 📏 Tính tốc độ hiện tại của nhân vật
...
-- 🧠 Kiểm tra pet có phải là bản ghost trong plot không
local function IsGhostPet(pet)
	-- Nếu pet đang nằm trong plot và cũng tồn tại 1 bản trùng tên trong workspace → là ghost
	if not pet or not pet:IsDescendantOf(workspace) then return false end
	local parentModel = pet:FindFirstAncestorWhichIsA("Model")
	if parentModel and parentModel.Name ~= "Plots" then
		-- pet nằm trong plot thật, không phải ghost
		return false
	end

	for _, obj in ipairs(workspace:GetChildren()) do
		if obj.Name == pet.Name and obj ~= pet then
			-- có bản khác cùng tên ở workspace => pet này chỉ là bóng mờ
			return true
		end
	end
	return false
...
local LastHeldPet = nil

local function HandlePet(pet, myPlot)
	if not pet or not pet:IsDescendantOf(workspace) then return end
	if IsPetInFriendPlot(pet) then 
		print("[⚠️] Bỏ qua pet trong plot của acc khác:", pet.Name)
		return 
	end
	if StolenPets[pet] then
		print("[🚫] Pet đã bị đánh dấu bị giành:", pet.Name)
		return
	end

	-- 🧠 Nếu pet có bản clone ở workspace → người khác đang cầm → bỏ qua
	for _, obj in ipairs(workspace:GetChildren()) do
		if obj.Name == pet.Name and obj ~= pet then
			print("[👻] Bỏ qua pet có clone ngoài workspace (đã bị người khác cầm):", pet.Name)
			return
		end
	end
	...
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		...
		-- 🔍 Sau khi hold, kiểm tra lại có bản clone ngoài workspace không
		...
		-- 🧩 Nếu pet vừa được cầm thành công → chính acc này là người tạo clone → KHÔNG coi là bị giành
		local stillHolding = false
		if cloneFound and LastHeldPet == pet then
			stillHolding = true
		end

		if cloneFound and not stillHolding then
			-- ❌ Clone có sẵn và mình không phải người tạo nó → bị giành
			print("[❌] Pet bị người khác giành:", pet.Name)
			statusText.Text = "Status : Pet bị giành → tìm con khác"
			StolenPets[pet] = true
			task.wait(1)
			return
		else
			-- ✅ Chính acc này là người vừa tạo clone → đang cầm thật
...
local function ScanAllPlots()
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		-- Nếu plot là của acc khác trong AccHold thì bỏ qua
		local isFriendPlot = false
		for _, cfg in ipairs(AccHold) do
			if cfg.AccountName == plot.Name and cfg.AccountName ~= player.Name then
				isFriendPlot = true
				break
			end
		end
		if isFriendPlot then
			continue
		end

		-- Quét pet trong plot hợp lệ
...
local function CheckMyPlotEmpty()
	...
	-- Kiểm tra nếu có pet trong plot
	...
	-- Nếu không còn pet trong plot nhưng có pet đang ở workspace → không kick
	for _, obj in ipairs(workspace:GetChildren()) do
		for _, cfg in ipairs(AccHold) do
			if obj.Name == cfg.Pet then
				return false
			end
		end
	end
...
UpdateUI()
print("✅ Pet Farm UI + Script loaded with Speed Coil!")
