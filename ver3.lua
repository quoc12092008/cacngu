...
-- ✅ FIX: Thêm tag "TakenBy" và kiểm tra trước khi lấy
local function HandlePet(pet, myPlot)
	-- Nếu pet đã có tag "TakenBy" → bỏ qua
	if pet:FindFirstChild("TakenBy") then 
		return 
	end

	-- Gắn tag ngay lập tức để acc khác không lấy
	local tag = Instance.new("StringValue")
	tag.Name = "TakenBy"
	tag.Value = player.Name
	tag.Parent = pet

	-- Tự động xóa tag sau khi hoàn thành
	task.spawn(function()
		task.wait(HoldTime + 5)
...
-- ✅ FIX: Chỉ quét pet ở plots khác nếu nhà mình chưa có
local function ScanAllPlots()
	local myPlot = GetMyPlot()
	if not myPlot then return end

	-- Kiểm tra xem nhà mình đã có pet cần farm chưa
	local myPets = GetPetsInPlot(myPlot)
	for _, name in ipairs(myPets) do
		if name == CurrentPet then
			-- Đã có pet trong nhà → dừng farm
			statusText.Text = "Status : Pet already in plot"
			return
		end
	end
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			for _, pet in ipairs(plot:GetChildren()) do
				-- Chỉ lấy pet đúng tên và chưa có tag "TakenBy"
				if pet.Name == CurrentPet and not pet:FindFirstChild("TakenBy") then
