...
local function GetPetsInPlot(plot)
	...
	-- Tìm tất cả các object trong plot
	for _, obj in ipairs(plot:GetChildren()) do
		-- Bỏ qua các folder/part không phải pet
		local skipNames = {"Decorations", "Floor", "Walls", "Roof", "SpawnLocation", "Door"}
		local skip = false
		...
		-- Nếu là Model và không phải các object cơ bản → có thể là pet
		if not skip and obj:IsA("Model") then
