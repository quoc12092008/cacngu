...
local function HandlePet(pet, myPlot)
	...
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		...
		-- Ghi lại vị trí pet ban đầu
		local oldPos = pet.WorldPivot and pet.WorldPivot.Position or nil
		...
		-- ✅ Kiểm tra xem pet có di chuyển hoặc biến mất không
		local stillThere = pet and pet:IsDescendantOf(workspace)
		local movedEnough = false
		if stillThere and oldPos then
			local newPos = pet.WorldPivot and pet.WorldPivot.Position or nil
			if newPos and (oldPos - newPos).Magnitude > 5 then
				movedEnough = true
			end
		end

		if (not stillThere) or movedEnough then
			-- 🟢 Thành công, pet đã được cầm
			print("[✅] Đã nhặt pet:", pet.Name)
			...
			-- ❌ Pet không di chuyển hoặc vẫn còn nguyên chỗ → bị giành
			print("[❌] Pet bị giành:", pet.Name)
