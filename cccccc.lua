...
<head>
    ...
    <style>
        ...
        .btn-primary:hover { box-shadow: 0 0 30px rgba(79, 70, 229, 0.5); transform: translateY(-1px); }
...
<body class="min-h-screen">
            ...
            <div class="page-transition">
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-float">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button onclick="showBackupModal()" class="glass-card px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-300 hover:text-white hover:bg-white/5 flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                            Backup
                        </button>
                        <button onclick="refreshData()" class="btn-primary btn-modern px-5 py-2.5 rounded-xl text-sm font-bold text-white flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <!-- NEW: Total Candy Card -->
                    <div class="glass-card p-5 rounded-2xl relative overflow-hidden group">
                        <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition group-hover:scale-110 transform">
                            <svg class="w-16 h-16 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        </div>
                        <p class="text-gray-400 text-xs font-bold uppercase tracking-wider">Total Candy</p>
                        <div class="flex items-baseline mt-2">
                            <span class="text-3xl font-bold text-pink-400" id="totalCandy">0</span>
                            <span class="ml-2 text-xs text-gray-500">🍬 CandyCane</span>
                        </div>
                        <p class="text-xs text-gray-500 mt-2"><span id="accountsWithCandy" class="text-white font-semibold">0</span> accounts with candy</p>
                    </div>
        ...
        <div id="accountsSection">
                ...
                <div class="flex flex-col lg:flex-row gap-4 justify-between mb-6">
                    ...
                    <div class="flex items-center gap-2 overflow-x-auto pb-2 lg:pb-0">
                        <button onclick="togglePetFilter()" class="px-4 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-sm font-medium text-gray-300 transition flex items-center gap-2 whitespace-nowrap">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
                            Advanced Filter
                        </button>
                        <div class="h-8 w-px bg-white/10 mx-1"></div>
                        <button onclick="selectAllFiltered()" class="px-4 py-2.5 hover:bg-indigo-500/20 text-indigo-400 rounded-xl text-sm font-medium transition whitespace-nowrap">Select All</button>
                        <button onclick="deleteSelected()" class="btn-danger px-4 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 whitespace-nowrap">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            Delete
                        </button>
                        <div class="relative group">
                            <button class="px-4 py-2.5 bg-emerald-600/10 text-emerald-400 hover:bg-emerald-600/20 border border-emerald-600/30 rounded-xl text-sm font-medium transition flex items-center gap-2 whitespace-nowrap">
                                Export
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </button>
                            <div class="absolute right-0 mt-2 w-48 bg-dark-800 border border-dark-border rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 transform translate-y-2 group-hover:translate-y-0">
                                <a href="#" onclick="copyFilteredAccounts()" class="block px-4 py-3 text-sm text-gray-300 hover:bg-white/5 first:rounded-t-xl">Only with Pets</a>
                                <a href="#" onclick="exportAllFilteredAccounts()" class="block px-4 py-3 text-sm text-gray-300 hover:bg-white/5 last:rounded-b-xl">All Accounts</a>
                            </div>
                        </div>
                    </div>
                </div>

                <div id="petFilterBox" class="hidden mb-6 bg-black/20 p-4 rounded-xl border border-white/5">
                    <p class="text-xs font-bold text-gray-500 uppercase mb-3">Filter by Pet Type</p>
                    <div class="flex gap-2 mb-3">
                        <select id="petFilterSelect" multiple class="modern-input w-full rounded-lg p-2 text-sm h-32"></select>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="applyPetFilter()" class="px-4 py-2 bg-indigo-600 rounded-lg text-sm font-semibold text-white hover:bg-indigo-500">Apply Filter</button>
                        <button onclick="clearPetFilter()" class="px-4 py-2 bg-transparent border border-white/10 rounded-lg text-sm text-gray-400 hover:text-white">Clear</button>
                    </div>
                </div>
                ...
                <!-- Action Buttons -->
                <div class="flex flex-wrap gap-4 mb-8 mt-10 border-t border-white/10 pt-8">
                    
                    <button onclick="copyFilteredAccounts()" class="bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white px-6 py-3 rounded-xl text-sm font-bold shadow-lg hover:shadow-orange-500/30 transition-all duration-300">
                        Export Accounts With Pets
                    </button>

                    <button onclick="exportAllFilteredAccounts()" class="btn-primary px-6 py-3 rounded-xl text-sm font-semibold">
                        Export All Filtered Accounts
                    </button>

                    <button onclick="selectAllFiltered()" class="bg-purple-600 hover:bg-purple-500 px-6 py-3 rounded-xl text-white font-semibold text-sm transition-all duration-300 shadow-lg hover:shadow-purple-500/30">
                        Select All Filtered
                    </button>

                    <button onclick="deleteSelected()" class="btn-danger px-6 py-3 rounded-xl text-sm font-semibold">
                        Delete Selected Accounts
                    </button>

                    <button onclick="clearAllData()" class="bg-gray-700 hover:bg-gray-600 px-6 py-3 rounded-xl text-white font-semibold text-sm transition-all duration-300 border border-white/10">
                        Clear All Data
                    </button>
                </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left py-5 px-6 text-gray-300 font-bold">Candy</th>
    ...
    <script>
        ...
        const ONLINE_THRESHOLD = 30 * 1000;  // 60 giây
            ...
            Object.values(accountsData).forEach(account => {
                if (account.pets) {
                    account.pets.forEach(pet => {
                        petTypes[pet.name] = (petTypes[pet.name] || 0) + 1;
                    });
                }
            });
            
            const summaryEl = document.getElementById('petSummary');
            if (Object.keys(petTypes).length === 0) {
                summaryEl.innerHTML = '<div class="text-gray-500 col-span-full text-center py-8">No pets found</div>';
                summaryEl.className = "flex justify-center"; // Center text
                return;
            }
            
            // Trả lại class Grid
            summaryEl.className = "grid grid-cols-2 md:grid-cols-4 gap-4";
            
            summaryEl.innerHTML = Object.entries(petTypes)
                .sort(([,a], [,b]) => b - a)
                .map(([name, count]) => `
                    <div onclick="showPetOwners('${name}')" class="cursor-pointer glass-card rounded-xl p-4 hover:scale-105 transition-all duration-300 border border-gray-600/30 hover:border-indigo-500/50 group relative">
                        <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition text-[10px] text-indigo-300">Click to view</div>
                        
                        <div class="text-gray-300 font-medium text-sm mb-1 truncate" title="${name}">${name}</div>
                        <div class="text-2xl md:text-3xl font-black text-emerald-400">${count}</div>
                    </div>
                `).join('');
        }
        ...
        function renderAccountsList() {
            ...
            const mobileListEl = document.getElementById('mobileAccountsList');

            if (pageAccounts.length === 0) {
                listEl.innerHTML = '<tr><td colspan="6" class="text-center py-12 text-gray-500">No accounts found</td></tr>';
                mobileListEl.innerHTML = '<div class="text-center py-12 text-gray-500">No accounts found</div>';
                return;
            }
            ...
            listEl.innerHTML = pageAccounts.map(([name, account]) => {
                ...
                <tr class="hover:bg-white/5 transition-all border-b border-white/5 last:border-0">
                    ...
                    <td class="py-4 px-6 align-top pt-6">
                        <span class="text-pink-400 font-bold font-mono">
                            ${account.candy ? account.candy.toLocaleString() : '0'} 🍬
                        </span>
                    </td>
