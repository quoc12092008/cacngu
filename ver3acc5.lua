...
-- Lấy PlotController (với error handling)
...
if not success then
	warn("⚠️ Không thể load PlotController:", err)
	player:Kick("Game chưa load xong, vui lòng thử lại!")
	return
end
-- 🔒 Tạo bảng lock pet toàn cục (anti trùng pet)
...
local function HandlePet(pet, myPlot)
	...
	-- 🔒 Nếu pet đang bị acc khác claim thì bỏ qua
	if shared.PetLocks[pet.Name] and shared.PetLocks[pet.Name] ~= player.Name then
		print("❌", player.Name, "bỏ qua", pet.Name, "(đã bị", shared.PetLocks[pet.Name], "claim trước)")
		return
	end

	-- 🔐 Đánh dấu pet này đang được acc hiện tại claim
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			...
			if owner and owner.Value then
				-- 🛑 Nếu plot thuộc acc đồng đội thì bỏ qua
				for _, acc in ipairs(AccHold) do
					if owner.Value == acc.AccountName then
						goto continuePlot
					end
				end
			end

			-- 🟢 Chỉ xử lý pet trùng tên CurrentPet
...
local function GetPetsInPlot(plot)
	...
	-- Danh sách loại trừ (không phải pet)
	local skipList = {
