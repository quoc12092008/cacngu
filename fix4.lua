...
print("[📝] Current AccHold Config:")
for i, cfg in ipairs(AccHold) do
	print("  " .. i .. ". AccountName: " .. cfg.AccountName .. " | Pet: " .. cfg.Pet)
end
...
local function GetPlotOwner(plot)
	...
	-- Cách đơn giản: lấy từ tên plot (ví dụ: "Checker_Plot" hoặc "Farm1_Plot")
	-- Hoặc parse từ structure nếu có
	...
	-- Nếu ko tìm được, assume là checker plot
	return "Checker"
end
...
local function ScanAllPlots()
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			-- ✅ BƯỚC MỚI: Kiểm tra xem plot này có phải farm plot không
			-- Nếu là farm plot (có người farm khác) → bỏ qua
			local plotOwner = GetPlotOwner(plot)
			if IsFarmAccount(plotOwner) then
				print("⛔ Plot " .. plot.Name .. " của acc farm khác → bỏ qua")
				continue
			end
			
			-- Nếu plot này là CHECKER → Tìm pet chưa bị lock
			local petCount = 0
				...
				if pet.Name == CurrentPet then
					...
					if not isLocked then
						...
						if petCount == farmAccIndex then
							print("[✅] " .. player.Name .. " (Farm#" .. farmAccIndex .. ") → Lấy pet#" .. petCount)
							HandlePet(pet, myPlot)
							return  -- Lấy xong 1 con → thoát, chờ vòng scsan tiếp
						end
					else
