...
local EMPTY_PLOT_CACHE_TTL = 30  -- Cache empty plot 30s rồi re-check
...
local collectedPets = {}
local skippedPets = {}
local emptyPlotCache = {}  -- { [plotName] = { isEmpty = bool, checkedAt = tick() } }
...
local function IsPlotEmpty(plot)
    if not plot or not plot.Parent then return true end
    
    local plotName = plot.Name
    
    -- Check cache trước
    local cached = emptyPlotCache[plotName]
    if cached and (tick() - cached.checkedAt) < EMPTY_PLOT_CACHE_TTL then
        return cached.isEmpty
    end
    
    -- Đọc PlotSign → SurfaceGui → Frame → TextLabel → Text
    local isEmpty = true
    local signText = ""
    ...
    local success = pcall(function()
        ...
        signText = textLabel.Text or ""
    end)
    
    if success and signText ~= "" then
        if string.find(signText, "Empty Base") then
            isEmpty = true
        else
            -- Có tên player thật, vd: "LOGAN_Duck200589's Base"
            isEmpty = false
        end
    end
    
    -- Cache kết quả
    emptyPlotCache[plotName] = {
        isEmpty = isEmpty,
        checkedAt = tick()
    }
    
    if isEmpty then
        log("👻", string.format("Empty plot: %s (\"%s\") → SKIP ALL PETS", plotName, signText))
    else
        log("🏠", string.format("Real plot: %s (\"%s\")", plotName, signText))
    end
...
local function FindTargetPet()
    ...
    for _, plot in ipairs(plotsFolder:GetChildren()) do
        if plot:IsA("Model") and plot ~= myPlot then
            -- Check PlotSign trước - skip base ảo (Empty Base)
