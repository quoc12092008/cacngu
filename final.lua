...
local function CheckMyPlotEmpty()
	...
	for _, pet in ipairs(myPlot:GetChildren()) do
		...
		for _, cfg in ipairs(AccHold) do
            ...
            if pet.Name == cfg.Pet and cfg.AccountName == player.Name then
                print(("[DEBUG] 🐾 Vẫn còn pet hợp lệ của acc này: %s"):format(pet.Name))
                return false -- còn pet hợp lệ của acc này
            end
		end
	end
