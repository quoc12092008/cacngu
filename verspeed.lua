...
local function BuySpeedCoil()
	if not remoteFunction then
		warn("⚠️ Không tìm thấy RemoteFunction:", remotePath)
		return false
	end
...
local function AdjustCameraBehindPlayer(pet)
	...
	cam.CFrame = CFrame.new(camPos, hrp.Position + Vector2.new(0, 2, 0))
...
local function InitializeFirstPet()
	local myPlot = GetMyPlot()
	if not myPlot then 
		warn("[⚠️] Không tìm thấy plot!")
		return false
	end
	
	-- 🎯 Bước 1: Tìm pet đầu tiên trong plot khác
	statusText.Text = "Status : Finding first pet..."
	local plotsFolder = workspace:FindFirstChild("Plots")
	if not plotsFolder then return false end
	
	local firstPet = nil
	local firstPetPlot = nil
	...
	for _, plot in ipairs(plotsFolder:GetChildren()) do
		if plot:IsA("Model") and plot ~= myPlot then
			...
			if firstPet then break end
		end
	end
	
	if not firstPet then
		warn("[⚠️] Không tìm thấy pet nào!")
		return false
	end
	
	-- 🎯 Bước 2: Đi lấy pet đầu tiên
	statusText.Text = "Status : Getting first pet..."
	local targetPos = firstPet.WorldPivot and firstPet.WorldPivot.Position
	if not targetPos then return false end
	
	if WalkToPosition(targetPos + Vector3.new(0, 2, 0)) then
		AdjustCameraBehindPlayer(firstPet)
		statusText.Text = "Status : Holding E on first pet..."
		HoldKeyEReal(HoldTime)
	else
		return false
	end
	...
	local homePos = GetHomeSpawn(myPlot)
	if not homePos then
		warn("[⚠️] Không tìm thấy home spawn!")
		return false
	end
	
	statusText.Text = "Status : Returning home with pet..."
	...
	-- 🎯 Bước 4: Đi đến AnimalPodiums Claim
	local claimPos = GetAnimalPodiumClaim(myPlot)
	if not claimPos then
		warn("[⚠️] Không tìm thấy AnimalPodiums Claim!")
		return false
	end
	
	statusText.Text = "Status : Going to Podium Claim..."
	if not WalkToPosition(claimPos + Vector3.new(0, 2, 0)) then
		return false
	end
	
	task.wait(0.5)
	
	-- 🎯 Bước 5: Mua Speed Coil
...
if CurrentPet then
	...
	-- 🎯 Khởi tạo: Lấy pet đầu tiên → về nhà → đến podium → mua Speed Coil
	task.spawn(function()
		task.wait(2)
		local success = InitializeFirstPet()
		
		if success then
			statusText.Text = "Status : Farming " .. CurrentPet
			
			-- Farm loop chỉ chạy sau khi khởi tạo xong
			task.spawn(function()
				while true do
					pcall(function()
